...
namespace GDBulletML
{
	...
	public abstract partial class Bullet : RefCounted, IBullet
	{
		...
		/// </summary>
		public List<BulletMLTask> BulletTasks { get; private set; }
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="GDBulletML.Bullet"/> class.
        /// </summary>
        ...
        protected Bullet(IBulletManager myBulletManager)
		{
			...
			BulletTasks = new List<BulletMLTask>();
        ...
        /// <summary>
        /// Recursively finds the <see cref="BulletMLTask"/> 
        /// </summary>
        /// <returns>The <see cref="BulletMLTask"/> matching <paramref name="labelStr"/></returns>
        /// <param name="labelStr">String label.</param>
        public BulletMLTask FindTaskByLabel<T>(string labelStr) where T : BulletMLTask
		{
			// Check each childTask for the correct task
			foreach (var childTask in BulletTasks)
			{
				// Try to get the task from our child node
				var foundTask = childTask.FindTaskByLabel<T>(labelStr);
				if (foundTask != default(BulletMLTask))
				{
					return (T)foundTask;
				}
			}

			// Didn't find the task, doesn't exist
			return default;
		}

		/// <summary>
		/// Find the matching <see cref="BulletMLTask"/> given, <paramref name="labelStr"/> and <paramref name="nodeName"/>
		/// </summary>
		...
		public BulletMLTask FindTaskByLabelAndName<T>(string labelStr, ENodeName nodeName) where T : BulletMLTask
		{
			// Check children tasks for a task matching the given label, name
			foreach (BulletMLTask childTask in BulletTasks)
			{
				BulletMLTask foundTask = childTask.FindTaskByLabelAndName<T>(labelStr, nodeName);
